...
class W3DIPMultiPatch(nn.Module):
    def __init__(
        ...
        super(W3DIPMultiPatch, self).__init__()
        self.kernel_gen = kernel_gen
    ...
    def forward(self) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        sharp_img_estimates = torch.cat(tuple(image_generator() for image_generator in self.image_generators))
        blur_kernel_estimate = self.kernel_gen().view(-1, 1, * self.kernel_gen.get_estimated_kernel_size())
        blurr_img_estimates = torch.cat(
            tuple(nn.functional.conv3d(sharp_img_estimate, blur_kernel_estimate, padding='same', bias=None)[None, ]
